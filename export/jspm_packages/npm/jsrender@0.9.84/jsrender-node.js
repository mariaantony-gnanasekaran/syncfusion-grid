...
(function(global) {
  ...
  $views = {
    ...
    sub: {
      ...
      onStore: {template: function(name, item) {
          if (item === null) {
            delete $render[name];
          } else {
            $render[name] = item;
  ...
  function getHelper(helper, isContextCb) {
    ...
    if (res && res._cp) {
      if (isContextCb) {
        deps = res[1] ? $sub._ceo(res[1].deps) : ["_jsvCp"];
        deps.unshift(res[0]);
        deps._cp = res._cp;
        return deps;
      }
  ...
  function convertArgs(converter, bound) {
    var l,
        key,
        boundArgs,
        tag = this,
    ...
    if (!args.length && tag.argDefault !== false && !tagCtx.index) {
      args = [tagCtx.view.data];
    } else if (converter && !bound) {
      args = args.slice();
    }
    if (bindTo) {
      boundArgs = [];
  ...
  function createCtxPrm(key, ctxPrmName, bindTo) {
    var tagCtx = this;
    ...
    function parseTag(all, bind, tagName, converter, colon, html, codeTag, params, slash, bind2, closeBlock, index) {
      ...
      if (codeTag) {
        ...
        if (tagName === "else") {
          ...
          current[9] = markup.substring(current[9], index);
          current = stack.pop();
          content = current[2];
          block = true;
        }
        ...
        newNode = [tagName, converter || !!convertBack || hasHandlers || "", block && [], parsedParam(paramsArgs || (tagName === ":" ? "'#data'," : ""), paramsProps, paramsCtxProps), parsedParam(args || (tagName === ":" ? "data," : ""), props, ctxProps), onError, useTrigger, late, pathBindings || 0];
    ...
    for (i = 0; i < l; i++) {
      ...
      if ("" + node === node) {
        ...
        if (tagName === "*") {
          ...
          } else if (tmplBindings && (pathBindings = node[8])) {
